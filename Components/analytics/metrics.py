...
def logging(args, logs_path, results_list, train_loss, val_loss, acc_list, epoch, fold_id, name):
    ...
    if epoch % args.save_interval == 0 and epoch > 1:
        ...
        if epoch == args.n_epochs:
            ...
            plots.create_acc_plot(args, logs_path, name, fold_id, epoch, acc_list, results_list['F1'], save=True, show=False)
        else:

            plots.create_loss_plot(args, train_loss, val_loss, logs_path, epoch, fold_id, name, save=False, show=False)
            plots.create_acc_plot(args, logs_path, name, fold_id,epoch, acc_list, save=False, show=False)
